...
SVN_PRIMARY_COMMIT_REVNUMS = 'cvs2svn-svn-primary-commit-revnums.db'
...
class CommitMapper(Singleton):
  ...
  def init(self, ctx):
    ...
    self.svn_primary_commit_revnums = Database(SVN_PRIMARY_COMMIT_REVNUMS, 'c')
    Cleanup().register(SVN_PRIMARY_COMMIT_REVNUMS, pass8)
  ...
  def get_svn_commit(self, svn_revnum):
    ...
    primary_revnum = self.svn_primary_commit_revnums.get(str(svn_revnum), None)
    if primary_revnum:
      primary_revnum = int(primary_revnum)
      svn_commit.set_primary_commit_revnum(primary_revnum)
  ...
  def set_primary_commit_revnum(self, svn_revnum, primary_revnum):
    """Store PRIMARY_REVNUM as the value of SVN_REVNUM"""
    self.svn_primary_commit_revnums[str(svn_revnum)] = str(primary_revnum)
...
class CVSCommit:
  ...
  def __init__(self, ctx, digest, author, log):
    ...
    self.primary_commit_revnum = SVN_INVALID_REVNUM
...
class SVNCommit:
  ...
  def __init__(self, ctx, description="", revnum=None, cvs_revs=None):
    ...
    # to refer to the same primary commit revision number, and it is
    # possible for a single synchronization commit to contain
    # CVSRevisions on multiple different default branches.
    self.primary_commit_revnum = None
  ...
  def set_primary_commit_revnum(self, revnum):
    "Set self.primary_commit_revnum to REVNUM."
    self.primary_commit_revnum = revnum
    CommitMapper(self._ctx).set_primary_commit_revnum(self.revnum, revnum)
