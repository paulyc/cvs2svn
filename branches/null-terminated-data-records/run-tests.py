...
def ctrl_char_in_filename():
  "allow control characters in filenames"

  control_chars = ''.join([chr(x) for x in (range(1,32) + [127])])

  # Strip out characters that are not allowed in the filesystem being used:
  illegal_chars = ''.join([chr(x) for x in []])
  for c in illegal_chars:
    control_chars = control_chars.replace(c, '')
  ...
  try:
    ...
    try:
      shutil.copyfile(os.path.join(base_path, 'twoquick,v'),
                      os.path.join(base_path, 'f' + control_chars + 'x,v'))
      new_path = os.path.join(dstrepos_path, 'd' + control_chars + 'x')
      os.rename(base_path, new_path)
    except:
      # Operating systems that don't allow control characters in filenames
      # will hopefully have thrown an exception.
      raise svntest.Skip

    # if ensure_conversion can generate a
    conv = ensure_conversion('ctrl-char-filename')
  finally:
    svntest.main.safe_rmtree(dstrepos_path)
